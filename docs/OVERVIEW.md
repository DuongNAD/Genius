...
| Tổng số test thu thập | **918 (915 pass, 3 skip)** |
| Lint `flake8` | ✅ 0 lỗi |
...
- **`db.py`** — SQLite (WAL; không dùng `auto_vacuum`), 3 bảng: `conversations`, `agent_logs`, `seen_jtis`, cùng index `agent_logs(task_id)` và `(agent_name, status)`. Bảng lịch sử `conversations`/`agent_logs` có retention giới hạn (`GENIUS_DB_MAX_ROWS`/`GENIUS_DB_RETENTION_DAYS`, prune lúc `init_db`). **Hàng đợi ghi đơn luồng** (`SQLiteWriterThread`) tuần tự hoá mọi write để tránh tranh chấp, nhưng cho ngữ nghĩa đồng bộ (block chờ Event, re-raise lỗi).
...
- **Số lượng:** **918 test thu thập (915 pass, 3 skip)**, ~2 phút. Các cụm lớn: `test_e2e.py` 74, `tests/test_distributed.py` 71, `test_e2e_phase5.py` 40; R2–R4 bổ sung `tests/test_upgrades.py`, `test_repo_graph.py`, `test_agent_factory.py`, `test_code_graph_index.py`, `test_mcp_code_graph.py`, `test_cast_chunking.py`, `tests/test_realrun_mcp.py` (stdio subprocess thật, ghim chính xác danh sách 18 tool), `tests/test_notebooklm.py` (backend + tool NotebookLM), `tests/test_realrun_hmac.py`.
...
- **Runtime:** pydantic 2.13.4 (pin trong requirements.txt — cập nhật doc này khi đổi pin), FastAPI, uvicorn, httpx, websockets 16, tenacity, PyYAML, python-dotenv, tiktoken, pathspec, tree-sitter (+ grammar js/ts/go — dùng grammar chính chủ, KHÔNG dùng `tree-sitter-language-pack` 1.x vì đã đổi sang binding PyO3 không tài liệu), pytest + pytest-asyncio + pytest-timeout. `requirements.txt` **pin đầy đủ** (3 máy + CI cài y hệt nhau); nhánh `mac` thêm đúng một dòng `uvloop` (không bao giờ merge dòng này sang `win`/`main`).
...
- **Văn hoá test mạnh:** 918 test với tiêm lỗi mạng thật (MockNetworkProtocol) + smoke test subprocess stdio MCP thật, mandate cấm giả lập kết quả.
