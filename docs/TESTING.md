...
- **540 test** thu thập, **100% pass** (0 fail), ~110s trên máy dev.
- **CI** (`.github/workflows/ci.yml`): `windows-latest`, Python 3.11, `pip install -r requirements.txt` → `python -m pytest`.
- **Lint** không nằm trong CI mà ở **pre-commit** (black 24.4.2 + flake8 7.0.0, pin trong `.pre-commit-config.yaml`).

> Các số liệu cũ trong `history/` (243 test, 114 test E2E…) là **snapshot lịch sử** ở các mốc trước, không phản ánh tổng hiện tại.
...
- `conftest.py`: seed mock API key; fixture autouse đổi `SKILL_API_KEY` theo tên file (`valid-api-key` cho `*distributed*`/`*robustness*`/`*milestone3_adversarial*`, còn lại `mock-skill-key`); monkeypatch cho phép plain-SHA256 cho test legacy (trừ `test_upgrades`); tắt debate & response-cache để chạy xác định.
