...
- Nâng cấp `VectorMemory`: Thay `SimpleTFIDFEmbedding` bằng `sentence-transformers` (vd: `all-MiniLM-L6-v2`) hoặc mô hình nhúng tương tự.
...
Thực hiện đánh giá toàn diện dự án Genius, bao gồm kiến trúc, chất lượng code, bảo mật và chất lượng các bài kiểm thử hiện tại.

Working directory: /Users/duongnad/Documents/project/Genius
Integrity mode: development

## Requirements

### R1. Đánh giá kiến trúc và chất lượng code
Tiến hành phân tích mã nguồn hệ thống để tìm ra các điểm thắt cổ chai về hiệu suất, các đoạn code chưa tối ưu và đánh giá cấu trúc tổng thể của dự án.

### R2. Kiểm tra bảo mật
Thực hiện rà soát các lỗ hổng bảo mật tiềm ẩn, đặc biệt chú ý đến cách hệ thống xử lý dữ liệu, phân quyền và các endpoint.

### R3. Đánh giá các bài kiểm thử (Testing)
Chạy bộ test hiện có (unit test, E2E test) và phân tích tính ổn định, độ bao phủ cũng như phát hiện các bài test có khả năng bị lỗi (flaky tests). 

## Acceptance Criteria

### Báo cáo đánh giá tổng thể
- [ ] Có một file báo cáo `evaluation_report.md` được tạo ra ở thư mục gốc của dự án.
- [ ] Báo cáo liệt kê kết quả chạy test tự động (pass/fail/error).
- [ ] Báo cáo chỉ ra ít nhất 3 vấn đề cụ thể về kiến trúc/chất lượng code cùng với đề xuất cải thiện.
- [ ] Báo cáo phân tích ít nhất 1 nguy cơ bảo mật hoặc điểm cấu hình cần được thắt chặt.
