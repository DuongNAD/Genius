...
def mock_providers():
    ...
    with patch("ag_core.providers.openai_provider.OpenAIProvider.send_prompt", new_callable=AsyncMock, return_value=mock_openai_res), \
         patch("ag_core.providers.anthropic_provider.AnthropicProvider.send_prompt", new_callable=AsyncMock, return_value=mock_anthropic_res):
        yield
...
def make_headers(jwt_token, payload_dict=None, raw_body=None, include_checksum=True, checksum_override=None, token_key="X-API-Key", bearer_style=False):
    headers = {}
    if jwt_token:
        if bearer_style:
...
@pytest.mark.parametrize("get_app,role", [(get_security_app, "security"), (get_devops_app, "devops")])
def test_auth_missing_token(get_app, role):
    ...
    # GET /status/123
    res_status = client.get("/status/123-abc", headers={"X-Payload-SHA256": hashlib.sha256(b"").hexdigest()})
...
def test_auth_invalid_token_format(get_app, role):
    ...
    app = get_app()
    client = TestClient(app)
    headers, body = make_headers(jwt_token="not.a.jwt.token", payload_dict={"prompt": "test"})
...
def test_auth_wrong_signature(get_app, role):
    ...
    wrong_token = encode_jwt({"sub": "orchestrator", "exp": time.time() + 300}, "wrong-secret-key")
...
def test_auth_expired_token(get_app, role):
    ...
    expired_token = encode_jwt({"sub": "orchestrator", "exp": time.time() - 600}, "test-key")
    
    headers, body = make_headers(jwt_token=expired_token, payload_dict={"prompt": "test"})
    res = client.post("/run", content=body, headers=headers)
    assert res.status_code == 401
...
def test_checksum_missing(get_app, role):
    ...
    headers, body = make_headers(jwt_token=jwt_token, payload_dict={"prompt": "test"}, include_checksum=False)
    res = client.post("/run", content=body, headers=headers)
...
def test_checksum_mismatch(get_app, role):
    ...
    headers, body = make_headers(jwt_token=jwt_token, payload_dict={"prompt": "test"}, checksum_override="wrongchecksumvalue")
    res = client.post("/run", content=body, headers=headers)
    assert res.status_code == 400
...
async def test_orchestrator_routing_execution(tmp_path):
    ...
    def mock_post(url, *args, **kwargs):
        ...
        resp.headers = httpx.Headers({"X-Payload-SHA256": hashlib.sha256(resp.content).hexdigest()})
    ...
    with patch("httpx.AsyncClient.post", side_effect=mock_post), \
        ...
        assert len(posted_urls) == 1
        assert "8006" in posted_urls[0] or "devops" in posted_urls[0] or "deploy" in posted_urls[0]
...
async def test_orchestrator_tenacity_retry_on_429():
    ...
    def mock_post(url, *args, **kwargs):
        ...
        if call_count < 3:
            ...
            resp.raise_for_status = lambda: (_ for _ in ()).throw(httpx.HTTPStatusError("Rate Limit", request=MagicMock(), response=resp))
        else:
            resp.status_code = 200
    ...
    with patch("httpx.AsyncClient.post", side_effect=mock_post), \
        ...
        # Make the call
        result = await orchestrator.call_api("http://localhost:8005", "api_key", "test prompt")
