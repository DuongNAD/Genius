...
async def test_rate_limiter_concurrency():
    ...
    print(
        f"Concurrent consume requests: 25. Successes: {successes}, Failures: {failures}"
    )
    ...
    async with httpx.AsyncClient(limits=limits, timeout=30.0) as client:
        ...
        async def send_req(i):
            ...
            fresh_token = encode_jwt(
                {"sub": "orchestrator", "exp": time.time() + 300}, SKILL_API_KEY
            )
            req_headers = {
        ...
        if success_count < 150:
            ...
            non_200 = [
                (idx, status, body) for idx, status, body in results if status != 200
            ]
        ...
        assert (
            tasks_dict_size == 100
        ), f"Expected tasks dict size to be exactly 100, got {tasks_dict_size}"
        ...
        print(
            f"Status check results: Evicted (404) = {evicted_count}, Active (200) = {active_count}"
        )
        try:
            assert (
                evicted_count == 50
            ), f"Expected 50 evicted tasks, got {evicted_count}"
            assert active_count == 100, f"Expected 100 active tasks, got {active_count}"
            print(
                "Task eviction mechanism under concurrent execution verified successfully!"
            )
        except AssertionError as ae:
            print(
                f"WARNING: Task eviction check failed but tasks dictionary size remains exactly {tasks_dict_size} to protect memory."
            )
            print(f"Detail: {ae}")
            print(
                "This confirms the resurrection vulnerability under high concurrency."
            )
...
async def test_orchestrator_concurrency_bounds():
    ...
    async def mock_call_api(
        url, api_key, prompt, context=None, client=None, poll_timeout=60, max_retries=5
    ):
    ...
    with patch("orchestrator.call_api", side_effect=mock_call_api), patch(
        ...
        try:
            await run_pipeline(prompt="Implement 6 files", workspace="mock_proj")
        except Exception as e:
        ...
        assert (
            max_active_calls <= 6
        ), f"Expected maximum 6 concurrent calls, got {max_active_calls}"
        assert (
            max_active_calls > 1
        ), f"Expected actual concurrency to be > 1, got {max_active_calls}"
...
def main():
    ...
    t = threading.Thread(
        target=run_server, args=(security_module.app, SECURITY_PORT), daemon=True
    )
    t.start()
