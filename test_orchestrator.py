...
async def test_pipeline_sequencing_and_success(mock_exec, mock_get, mock_post, temp_workspace):
    ...
    assert test_generated_path.read_text(encoding="utf-8") == "def test_generated(): pass"

@pytest.mark.asyncio
@patch("httpx.AsyncClient.post", new_callable=MagicMock)
@patch("httpx.AsyncClient.get", new_callable=MagicMock)
@patch("asyncio.create_subprocess_exec", new_callable=MagicMock)
async def test_pipeline_stdout_redirection(mock_exec, mock_get, mock_post, temp_workspace):
...
@patch("httpx.AsyncClient.get", new_callable=MagicMock)
@patch("asyncio.create_subprocess_exec", new_callable=MagicMock)
async def test_pipeline_early_exit_on_error(mock_exec, mock_get, mock_post, temp_workspace):
    ...
    assert "Task execution failed on server: Claude internal error" in str(exc_info.value)
...
@patch("httpx.AsyncClient.get", new_callable=MagicMock)
@patch("asyncio.create_subprocess_exec", new_callable=MagicMock)
async def test_pipeline_early_exit_on_missing_or_empty_output(mock_exec, mock_get, mock_post, temp_workspace):
...
def test_resolve_antigravity_cmd_env_override():
    ...
    with patch.dict(os.environ, {"ANTIGRAVITY_BIN_PATH": "/custom/path/to/antigravity"}):
        ...
        with patch("sys.platform", "win32"):
            with patch("os.path.exists", return_value=True) as mock_exists:
                with patch.dict(os.environ, {"USERPROFILE": "C:\\Users\\MockUser"}):
                    cmd = resolve_antigravity_cmd()
                    assert cmd == os.path.join("C:\\Users\\MockUser", ".gemini", "antigravity", "bin", "antigravity.cmd")
                    mock_exists.assert_any_call(os.path.join("C:\\Users\\MockUser", ".gemini", "antigravity", "bin", "antigravity.cmd"))
...
async def test_interactive_design_review_loop(mock_input, mock_get, mock_post, temp_workspace):
    async def post_side_effect(url, **kwargs):
        import hashlib
        import json
    ...
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        await run_pipeline(prompt="Build a game", workspace=str(temp_workspace), interactive=True)
...
@patch("httpx.AsyncClient.get", new_callable=MagicMock)
@patch("asyncio.create_subprocess_exec", new_callable=MagicMock)
async def test_self_healing_loop_success_after_retry(mock_exec, mock_get, mock_post, temp_workspace):
    async def post_side_effect(url, **kwargs):
        import hashlib
        import json
        if "8001" in str(url):
    ...
    await run_pipeline(prompt="Build a self healer", workspace=str(temp_workspace), max_retries=3)
