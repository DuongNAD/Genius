...
async def test_debate_flow_sequence_no_approval(mock_exec, mock_call_api, temp_workspace):
    ...
    async def mock_call_api_impl(url, api_key, prompt, context=None, client=None, poll_timeout=60.0):
        nonlocal call_idx
    ...
    assert "Claude design v3 (final)" in open(os.path.join(str(temp_workspace), "projects", "build_a_calculator_app", "design.md"), "r", encoding="utf-8").read()
...
async def test_debate_flow_instant_approval(mock_exec, mock_call_api, temp_workspace):
    ...
    design_content = open(os.path.join(str(temp_workspace), "projects", "build_a_calculator_app", "design.md"), "r", encoding="utf-8").read()
