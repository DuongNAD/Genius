...
def test_sqlite_fallback_store(temp_db):
    memory = VectorMemory(
        collection_name="test_collection", use_chroma=False, db_path=temp_db
    )

    # Insert items
    doc_id1 = memory.add(
        "Deploy docker containers and kubernetes pods", metadata={"category": "ops"}
    )
    doc_id2 = memory.add(
        "Python script to fetch data from API", metadata={"category": "dev"}
    )
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, collection_name, text, metadata FROM agent_vector_memory_fallback"
    )
...
async def test_agent_integration(temp_db):
    ...
    mock_provider.send_prompt = AsyncMock(
        return_value={
