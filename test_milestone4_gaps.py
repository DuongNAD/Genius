...
def test_orchestrator_cli_arguments():
    test_args = [
        ...
        "--prompt", "hello",
        "--security-cmd", "my-sec",
        "--security-args", "arg1", "arg2",
        "--devops-cmd", "my-dev",
        "--devops-args", "arg3",
        "--grok-url", "http://grok-override",
        "--claude-url", "http://claude-override",
        "--codex-url", "http://codex-override",
        "--tester-url", "http://tester-override",
        "--security-url", "http://security-override",
        "--devops-url", "http://devops-override",
        "--api-key", "my-api-override",
        "--poll-timeout", "120.0"
    ]
    
    with patch.object(sys, "argv", test_args), \
         patch("orchestrator.run_pipeline", new_callable=AsyncMock) as mock_run:
        try:
            main()
        except SystemExit:
            pass
