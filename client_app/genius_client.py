...
def generate_jwt(api_key: str) -> str:
    ...
    payload = {"sub": "orchestrator", "exp": int(time.time() + 300)}
    ...
    while True:
        try:
            ...
            if response.status_code == 429 or response.status_code >= 500:
                ...
                sys.stderr.write(
                    f"[INFO] Transient HTTP error {response.status_code} on attempt {attempt}. Retrying in {delay:.2f}s...\n"
                )
                sys.stderr.flush()
                time.sleep(delay)
            ...
            sys.stderr.write(
                f"[INFO] Transient error ({type(e).__name__}): {e} on attempt {attempt}. Retrying in {backoff:.2f}s...\n"
            )
            sys.stderr.flush()
            time.sleep(backoff)
            backoff *= backoff_factor
...
def main():
    ...
    parser.add_argument(
        "--port",
        type=int,
        help="Agent Port (Grok=8001, Claude=8002, Codex=8003, Tester=8004, Security=8005, DevOps=8006)",
    )
    ...
    parser.add_argument(
        "--timeout", type=float, default=120.0, help="Polling timeout in seconds"
    )
    parser.add_argument(
        "--max-retries", type=int, default=5, help="Max retries for transient errors"
    )
    ...
    if not ip:
        try:
            ...
            while not ip:
                ip = input(
                    "Server IP Address cannot be empty. Enter Server IP Address: "
                ).strip()
    ...
    if not port:
        try:
            port_str = input(
                "Enter Agent Port (Grok=8001, Claude=8002, Codex=8003, Tester=8004, Security=8005, DevOps=8006): "
            ).strip()
            while not port_str:
                port_str = input(
                    "Agent Port cannot be empty. Enter Agent Port: "
                ).strip()
            try:
                port = int(port_str)
            except ValueError:
