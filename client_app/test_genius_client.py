...
class TestGeniusClient(unittest.TestCase):
    ...
    def test_jwt_generation(self):
        ...
        expected_sig = hmac.new(
            api_key.encode("utf-8"), signing_input.encode("utf-8"), hashlib.sha256
        ).digest()
        expected_sig_b64 = (
    ...
    def test_get_endpoints(self):
        # Port 8005 (security)
        self.assertEqual(
            get_endpoints(8005), ("/security/run", "/security/status/{task_id}")
        )
        # Port 8006 (devops)
        self.assertEqual(
            get_endpoints(8006), ("/devops/run", "/devops/status/{task_id}")
        )
        # Port 8001 (grok)
    ...
    @patch("requests.post")
    def test_make_request_with_retry_transient_failure_then_success(
        ...
        response1.raise_for_status.side_effect = requests.exceptions.HTTPError(
            "Internal Server Error", response=response1
        )
        response1.headers = {}
    ...
    def test_run_client_success(self, mock_retry_request, mock_sleep):
        ...
        run_response.headers = {
            "X-Payload-SHA256": hashlib.sha256(run_content).hexdigest()
        }
        ...
        status_response1.headers = {
            "X-Payload-SHA256": hashlib.sha256(status_content1).hexdigest()
        }

        # 2. completed
        status_content2 = b'{"status":"completed","result":"my final result text"}'
        status_response2 = MagicMock()
        status_response2.json.return_value = {
            "status": "completed",
            "result": "my final result text",
        }
        status_response2.content = status_content2
        status_response2.headers = {
            "X-Payload-SHA256": hashlib.sha256(status_content2).hexdigest()
        }
    ...
    def test_run_client_task_failed(self, mock_retry_request, mock_sleep):
        ...
        status_response.headers = {
            "X-Payload-SHA256": hashlib.sha256(status_content).hexdigest()
        }
    ...
    @patch("sys.exit")
    def test_cli_args_interactive_fallback(
        self, mock_exit, mock_run_client, mock_input
    ):
