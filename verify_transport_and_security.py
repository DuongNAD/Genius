...
def main():
    ...
    try:
        token = encode_jwt({"sub": "worker-1", "exp": time.time() + 60}, "mock-skill-key")
    ...
    resp = sec_client.post("/run", content=payload_bytes, headers=valid_headers_no_checksum)
    print(f"A. Missing checksum header: Status = {resp.status_code}, Body = {resp.text}")
    ...
    resp = sec_client.post("/run", content=payload_bytes, headers=valid_headers_bad_checksum)
