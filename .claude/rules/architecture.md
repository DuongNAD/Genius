...
## Ports / roles
Hub `8000`, Researcher (role id `grok`) `8001`, Claude `8002`, Codex `8003`, Tester `8004`, Security `8005`, DevOps `8006`, Dashboard `8080`. Role ids, ports and service-registry keys are frozen — the `grok` role id stays even though the grok backend is opt-in. Service URLs come from `config.yaml`; a `.agents/service_registry.json` (dynamic port discovery) overrides them at load time.
