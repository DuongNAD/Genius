...
- **Model selection** — `config.models.<attr>` (attrs: `openai`, `anthropic`, `agy`; `grok` is cosmetic, never passed to the CLI). Empty = the CLI's own default model; a value is passed through verbatim (`codex -m`, `claude --model`, `agy --model`). Env override per BACKEND name (not config attr): **`GENIUS_MODEL_<BACKEND>`** — `GENIUS_MODEL_CODEX` (maps to `models.openai`), `GENIUS_MODEL_CLAUDE`, `GENIUS_MODEL_AGY`; blank = unset. Never default `openai` to a chat-API name like `gpt-4o` — the codex CLI rejects it.
