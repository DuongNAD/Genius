...
- The R5 eval knobs are also off under pytest by construction: the post-run eval gate (`orchestrator.eval_gate_enabled()` returns `False` under pytest regardless of `GENIUS_EVAL_GATE`) and the surgical-edit coder prompt (`GENIUS_SURGICAL_EDITS` unset in the suite). Same construction for dependency auto-install: `orchestrator.auto_install_enabled()` returns `False` under pytest regardless of `GENIUS_AUTO_INSTALL`, so the suite can never create a venv or hit pip; its tests (`tests/test_auto_install.py`) monkeypatch the flag/subprocess layer instead. Ditto stack-aware project gates: `orchestrator.project_gate_enabled()` is `False` under pytest regardless of `GENIUS_PROJECT_GATE` (`tests/test_project_gate.py` fakes `run_subprocess` — npm never runs in the suite). Test the eval layer directly instead: `ag_core.eval` (metrics/grader/judge/compare via an injected fake judge) and the MCP `eval` tool (`dispatch_tool("eval", ...)` with the deterministic default metrics — no provider needed). Adding an MCP tool still requires updating `tests/test_realrun_mcp.py::EXPECTED_TOOLS`.
