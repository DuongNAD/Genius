...
def test_agent_endpoints(app_getter, role_name, run_path, status_path_template):
    ...
    assert "Missing token" in resp.json()["detail"] or "Not authenticated" in resp.json()["detail"]
    
    # 2. Invalid Token -> 401
    headers_invalid = {"X-API-Key": "invalid-token", "Authorization": "Bearer invalid-token"}
    ...
    resp = client.post(run_path, json={"prompt": "test"}, headers=headers_valid_no_payload_sum)
    print(f"Missing X-Payload-SHA256 Header: Status = {resp.status_code}, Body = {resp.text}")
    ...
    print(f"Bad X-Payload-SHA256 Header: Status = {resp.status_code}, Body = {resp.text}")
    assert resp.status_code == 400
    ...
    payload_bytes_missing = json.dumps(body_missing, separators=(',', ':')).encode("utf-8")
    ...
    headers_invalid_json["X-Payload-SHA256"] = compute_sha256(payload_bytes_invalid_json)
    resp = client.post(run_path, content=payload_bytes_invalid_json, headers=headers_invalid_json)
    print(f"Invalid JSON body: Status = {resp.status_code}, Body = {resp.text}")
    assert resp.status_code in [400, 422]

    # 10. Valid POST request -> 200, checks Response Headers
    body_valid = {"prompt": "audit project" if role_name == "security" else "deploy project"}
    ...
    resp_get = client.get(status_path_template.format(task_id=task_id), headers=headers_get)
    ...
    resp_get_404 = client.get(status_path_template.format(task_id="nonexistent-task-id"), headers=headers_get)
    print(f"GET status 404: Status = {resp_get_404.status_code}, Body = {resp_get_404.text}")
...
def test_rate_limiter(app_getter, role_name):
    ...
    print(f"Rate Limited Request: Status = {resp.status_code}, Body = {resp.text}, Headers = {resp.headers}")
...
def verify_slash_commands_and_routing():
    ...
    for cmd, route in serve_expected_routing.items():
        assert serve.ROUTING_TABLE.get(cmd) == route, f"serve.ROUTING_TABLE mismatch for command {cmd}: expected {route}, got {serve.ROUTING_TABLE.get(cmd)}"
        
    for cmd, route in orchestrator_expected_routing.items():
        assert orchestrator.ROUTING_TABLE.get(cmd) == route, f"orchestrator.ROUTING_TABLE mismatch for command {cmd}: expected {route}, got {orchestrator.ROUTING_TABLE.get(cmd)}"
...
def main():
    try:
        ...
        test_agent_endpoints(get_security_app, "security", "/security/run", "/security/status/{task_id}")
        
        # 2. Test DevOps Agent Endpoints on Port 8006 paths
        test_agent_endpoints(get_devops_app, "devops", "/run", "/status/{task_id}")
        test_agent_endpoints(get_devops_app, "devops", "/devops/run", "/devops/status/{task_id}")
