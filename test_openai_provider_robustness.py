...
async def test_openai_provider_excessively_long_output():
    ...
    for i in range(10000):
        lines.append(
            json.dumps({"event": "agent_message", "item": {"text": f"part_{i} "}})
        )
    lines.append(
        json.dumps(
            {"event": "turn.completed", "input_tokens": 100, "output_tokens": 200}
        )
    )
    long_output = "\n".join(lines) + "\n"
...
async def test_openai_provider_non_zero_exit_code():
    ...
    with patch("asyncio.create_subprocess_exec", new_callable=AsyncMock) as mock_exec:
        ...
        assert "Codex CLI failed with exit code 1: Some CLI error output" in str(
            exc_info.value
        )
