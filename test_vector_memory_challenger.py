...
def test_edge_cases(temp_db_path):
    memory = VectorMemory(
        collection_name="edges", use_chroma=False, db_path=temp_db_path
    )
...
def test_scale_performance(temp_db_path):
    memory = VectorMemory(
        collection_name="scale", use_chroma=False, db_path=temp_db_path
    )
    ...
    print(
        f"Inserted {num_docs} documents in {insert_duration:.4f}s (avg {(insert_duration/num_docs)*1000:.2f}ms/doc)"
    )
...
def test_hashing_collisions(temp_db_path):
    ...
    with patch("ag_core.memory.vector_store.SENTENCE_TRANSFORMERS_AVAILABLE", False):
        memory = VectorMemory(
            collection_name="collision", use_chroma=False, db_path=temp_db_path
        )
...
def test_concurrent_access(temp_db_path):
    memory = VectorMemory(
        collection_name="concurrency", use_chroma=False, db_path=temp_db_path
    )
...
def test_wal_mode_concurrency_comparison(tmp_path):
    ...
    memory_def = VectorMemory(
        collection_name="default", use_chroma=False, db_path=db_default
    )
    ...
    print(
        f"[WAL Mode] Reader duration: {reader_results_wal[-1]:.4f}s, errors: {reader_errors_wal}"
    )

    # EXCLUSIVE Mode
    db_exclusive = str(tmp_path / "exclusive.db")
    memory_exc = VectorMemory(
        collection_name="exclusive", use_chroma=False, db_path=db_exclusive
    )
    ...
    print(
        f"[Default DELETE Mode - EXCLUSIVE Write Lock] Reader duration: {reader_results_exc[-1]:.4f}s, errors: {reader_errors_exc}"
    )

    # WAL Mode + EXCLUSIVE lock
    db_wal_exc = str(tmp_path / "wal_exc.db")
    memory_wal_exc = VectorMemory(
        collection_name="wal_exc", use_chroma=False, db_path=db_wal_exc
    )
    ...
    print(
        f"[WAL Mode - EXCLUSIVE Write Lock] Reader duration: {reader_results_wal_exc[-1]:.4f}s, errors: {reader_errors_wal_exc}"
    )

    assert (
        reader_results_exc[-1] >= 0.9
    ), f"Reader in EXCLUSIVE rollback mode didn't block! took {reader_results_exc[-1]}s"
    assert (
        reader_results_wal_exc[-1] < 0.2
    ), f"Reader in WAL mode blocked! took {reader_results_wal_exc[-1]}s"
...
def test_drive_offline_simulation():
    ...
    with patch("os.makedirs", side_effect=mock_makedirs), patch(
        "sqlite3.connect", side_effect=mock_connect
        ...
        try:
            db_utils.log_agent_start(
                "test-task-offline", "security_agent", "some prompt"
            )
        ...
        with pytest.raises((sqlite3.OperationalError, OSError)):
            VectorMemory(
                collection_name="offline_col",
                use_chroma=False,
                db_path="E:\\Project\\Genius\\offline_col.db",
            )
