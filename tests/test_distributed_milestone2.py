...
async def test_distributed_dispatch_and_execution_success(run_server):
    ...
    with patch("ag_core.agents.grok_researcher.GrokResearcherAgent.run", new_callable=AsyncMock) as mock_run:
...
async def test_distributed_dispatch_checksum_validation_failure():
    ...
    with patch("websockets.connect", mock_connect_2):
        worker_task_2 = asyncio.create_task(worker_2.run_production_loop("127.0.0.1", 8013))
    ...
    assert "Missing checksum validation on worker node" in result_msg_2["result"]["error"]
