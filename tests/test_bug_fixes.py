...
async def test_r2_codex_reviewer_path_traversal():
    ...
    with patch("ag_core.agents.codex_reviewer.log_transaction"), \
         patch.object(provider, "send_prompt", return_value=mock_response), \
...
async def test_r3_r4_windows_subprocess_and_length_limit():
    ...
    mock_process.communicate.return_value = (
        json.dumps({"result": "Hello", "usage": {"input_tokens": 1, "output_tokens": 1}}).encode("utf-8"),
        b""
    )
...
def test_r6_retry_after_http_date():
    ...
    headers = httpx.Headers({"Retry-After": "Wed, 21 Oct 2026 07:28:00 GMT"}) # future date
    response = httpx.Response(status_code=429, headers=headers, request=httpx.Request("GET", "http://test"))
    exception = httpx.HTTPStatusError("429 Too Many Requests", request=response.request, response=response)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            conn.execute(
                ...
                ("art-2", "stale_test", "version 2", "text", "test-process-2", art.timestamp + 10.0)
            )
            conn.commit()
            conn.close()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            with patch.dict("os.environ", {"GENIUS_DB_PATH": db_path}):
                ...
                def write_op(conn):
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO conversations (prompt, result) VALUES (?, ?)", ("q", "a"))
                    conn.commit()
...
async def test_r13_active_task_eviction():
    ...
    completed_task_ids = [tid for tid, tdata in mock_tasks.items() if tdata.get("status") in ("completed", "failed")]
...
async def test_r17_orchestrator_http_fallback():
    ...
    with patch("httpx.AsyncClient.post", side_effect=mock_post):
        ...
        try:
            res = await orchestrator.call_api("http://localhost:8001", "mock-key", "Test prompt", context={})
