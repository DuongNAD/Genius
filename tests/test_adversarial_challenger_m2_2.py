...
async def test_concurrent_dispatches_under_load(run_server):
    ...
    try:
        ...
        assert len(pending_tasks) == 0, f"Leaked futures in pending_tasks: {pending_tasks}"
...
async def test_worker_unexpected_websocket_close(run_server):
    ...
    async def run_flaky_worker():
        ...
        async with websockets.connect(f"ws://{HOST}:{port}/ws/connect?token={token}") as ws:
            ws_client = ws
            # Register
            reg_payload = {"type": "register", "worker_id": worker_id, "roles": ["grok"]}
...
async def test_result_payload_tampering_leaks_worker(run_server):
    ...
    w_info = await worker_registry.get_worker(worker_id)
    assert w_info["status"] == "idle", "Worker status should be cleaned up and set to idle!"
    
    # Under the fixed codebase, the task should be marked as failed
    task_id = list(serve_mod.central_hub.tasks.keys())[0]
    assert serve_mod.central_hub.tasks[task_id]["status"] == "failed", "Task should be marked as failed!"
