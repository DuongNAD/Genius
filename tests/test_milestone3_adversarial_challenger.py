...
from serve import app as serve_app, worker_registry, pending_tasks, central_hub, WorkerDisconnectedError, prune_stale_workers
import dashboard
...
class MockNetworkProtocol:
    ...
    async def send_to_hub(self, endpoint: str, payload: Any, headers: dict) -> tuple[int, Any]:
    ...
    async def send_to_worker(self, worker_id: str, endpoint: str, payload: Any, headers: dict) -> tuple[int, Any]:
...
def test_dashboard_status_distributed_direct_import():
    ...
    with patch.object(dashboard, "IS_DISTRIBUTED", True):
        ...
        with patch.dict("sys.modules", {"serve": MagicMock(worker_registry=mock_registry)}):
...
async def test_task_timeout_sweep_and_cancellation():
    ...
    cancel_requests = [
        req for req in net.request_log
...
async def test_late_result_reporting_hazard():
    ...
    status_code, response_body = await net.send_to_hub("/report_result", report_payload, report_headers)
...
async def test_live_websocket_concurrency_and_disconnects():
    ...
    prune_task = asyncio.create_task(prune_stale_workers(timeout_sec=0.5, check_interval=0.05))
    ...
    try:
        ...
        for i, fut in enumerate(task_futures):
            assert fut.done()
            with pytest.raises((WorkerDisconnectedError, asyncio.TimeoutError, Exception)):
                await fut
