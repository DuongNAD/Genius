...
async def call_api_with_retry(
    ...
    while True:
        ...
        headers = {"X-API-Key": api_key, "X-Payload-SHA256": checksum}
        try:
...
async def test_t1_f2_invalid_checksum_rejected(network, hub):
    ...
    headers["X-Payload-SHA256"] = (
        "wrongchecksumwrongchecksumwrongchecksumwrongchecksumwrongchecksum12"
    )
    status_code, body = await network.send_to_hub("/dispatch", payload, headers)
...
async def test_t1_f3_task_status_polling_during_execution(network, hub):
    ...
    status_code, status_body = await network.send_to_hub(
        "/task_status", poll_payload, poll_headers
    )
    assert status_code == 200
...
async def test_t1_f5_retry_on_transient_http_429(network, hub):
    ...
    status_code, body = await call_api_with_retry(
        network, "/dispatch", payload, hub.api_key
    )
    assert status_code == 202
    assert attempts == 2
...
async def test_t1_f5_exponential_backoff_timing(network, hub):
    ...
    await call_api_with_retry(
        network, "/dispatch", payload, hub.api_key, base_delay=0.01
    )
    t2 = time.time()
...
async def test_t1_f5_persistent_network_failure_raises_error(network, hub):
    ...
    with pytest.raises(asyncio.TimeoutError):
        await call_api_with_retry(
            network, "/dispatch", payload, hub.api_key, max_retries=2, base_delay=0.001
        )
...
async def test_t1_f6_workspace_state_sync_between_workers(network, hub):
    ...
    headers = hub.create_headers(payload)
    status_code, body = await network.send_to_hub(
...
async def test_t2_f5_zero_timeout_network_drop(network, hub):
    ...
    with pytest.raises(asyncio.TimeoutError):
        await call_api_with_retry(
            network, "/dispatch", payload, hub.api_key, max_retries=0, base_delay=0.001
        )
...
async def test_t2_f5_negative_backoff_config_fallback(network, hub):
    ...
    payload = {"role": "grok", "task_data": "data"}
    status_code, body = await call_api_with_retry(
        network, "/dispatch", payload, hub.api_key, max_retries=1, base_delay=-0.01
    )
    assert status_code == 202
...
async def test_t2_f5_mixed_errors_sequence_recovery(network, hub):
    ...
    payload = {"role": "grok", "task_data": "data"}
    status_code, body = await call_api_with_retry(
        network, "/dispatch", payload, hub.api_key, max_retries=4, base_delay=0.001
    )
    assert status_code == 202
    assert attempts == 3
...
async def test_t4_workload_1_multi_stage_agent_pipeline(network, hub):
    ...
    # Stage 1: Research
    headers = hub.create_headers(
        {"role": "grok_researcher", "task_data": "query_prompt"}
    )
    _, dispatch_res = await network.send_to_hub(
        "/dispatch", {"role": "grok_researcher", "task_data": "query_prompt"}, headers
    )
    ...
    headers = hub.create_headers(
        {"role": "claude_architect", "task_data": design_input}
    )
    _, dispatch_res2 = await network.send_to_hub(
        "/dispatch", {"role": "claude_architect", "task_data": design_input}, headers
    )
    ...
    _, dispatch_res3 = await network.send_to_hub(
        "/dispatch", {"role": "codex_reviewer", "task_data": review_input}, headers
    )
    ...
    assert (
        "Processed: Review design: Processed: Design based on Processed: query_prompt"
        in hub.tasks[t3_id]["result"]["output"]
    )
...
async def test_t4_workload_5_chaos_monkey_crash_recovery(network, hub):
    ...
    await network.send_to_hub(
        "/deregister", {"worker_id": "w1"}, hub.create_headers({"worker_id": "w1"})
    )
