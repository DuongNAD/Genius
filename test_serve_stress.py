...
async def test_duplicate_worker_id_registrations(run_server):
    ...
    assert not is_corrupted, "Registry got corrupted! Client A's disconnect unregistered Client B's registration."
...
async def test_heartbeat_pruning_timing(run_server):
    ...
    prune_task = asyncio.create_task(prune_stale_workers(timeout_sec=0.5, check_interval=0.1))
