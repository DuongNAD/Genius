...
def import_skill_api(skill_name):
    ...
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".agents", "skills", skill_name, "api.py")
...
def get_valid_api_key():
    ...
    config = load_config()
    secret = getattr(config, "skill_api_key", None) or os.getenv("SKILL_API_KEY", "mock-skill-key") or "mock-skill-key"
    payload = {
        "sub": "test",
...
def make_mock_http_response(status_code=200, json_data=None, content=None, headers=None):
...
def mock_llm_providers():
    ...
    with patch("ag_core.providers.openai_provider.OpenAIProvider.send_prompt", new_callable=AsyncMock, return_value=mock_res), \
         patch("ag_core.providers.anthropic_provider.AnthropicProvider.send_prompt", new_callable=AsyncMock, return_value=mock_res), \
         patch("ag_core.providers.grok_provider.GrokProvider.send_prompt", new_callable=AsyncMock, return_value=mock_res):
        yield
...
def test_f1_grok_server_startup():
    ...
    assert "/docs" in routes or "/run" in routes, "Grok API missing /docs or /run endpoints"
...
def test_f1_claude_server_startup():
    ...
    client = TestClient(api.app)
    routes = [r.path for r in api.app.routes]
    assert "/docs" in routes or "/run" in routes, "Claude API missing /docs or /run endpoints"
...
def test_f1_codex_server_startup():
    ...
    client = TestClient(api.app)
    routes = [r.path for r in api.app.routes]
    assert "/docs" in routes or "/run" in routes, "Codex API missing /docs or /run endpoints"
...
def test_f1_swagger_docs_accessible():
    ...
    assert "swagger" in res.text.lower() or "openapi" in res.text.lower() or "redoc" in res.text.lower()
...
def test_f2_grok_auth_success():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_post(client, "/run", headers={"X-API-Key": get_valid_api_key()}, json_data={"prompt": "hello"})
...
def test_f2_auth_invalid_key():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_post(client, "/run", headers={"X-API-Key": "wrong-key"}, json_data={"prompt": "hello"})
    assert res.status_code == 401
...
def test_f2_status_auth_grok():
    ...
    assert res.status_code == 401
    # Valid key
    res = client_get(client, "/status/task-123", headers={"X-API-Key": get_valid_api_key()})
...
def test_f3_payload_stateless_execution(tmp_path, monkeypatch):
    ...
    client_post(client, "/run", headers={"X-API-Key": get_valid_api_key()}, json_data={"prompt": "hello"})
...
def test_f3_payload_missing_required_field():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_post(client, "/run", headers={"X-API-Key": get_valid_api_key()}, json_data={"context": "some context"})
...
def test_f4_orchestrator_routes_requests():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "mock output"})
...
def test_f4_orchestrator_polls_status():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_get.side_effect = [
            ...
            make_mock_http_response(200, {"status": "completed", "result": "mock output"}),
        ] * 6
...
def test_f4_orchestrator_saves_outputs_locally():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "result data"})
        
        workspace_dir = "temp_workspace_f4"
        os.makedirs(workspace_dir, exist_ok=True)
        try:
            asyncio.run(run_pipeline(prompt="Save files", workspace=workspace_dir))
            for f in ["research.md", "design.md", "app.py", "review.md", "test_generated.py"]:
...
def test_f4_orchestrator_non_zero_exit_on_step_failure():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "failed", "error": "Internal Agent Error"})
...
def test_f5_orchestrator_retries_on_429():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        ] + [make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})] * 5
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "mock"})
...
def test_f5_orchestrator_max_retries_exhaustion():
    ...
    from orchestrator import run_pipeline
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        mock_post.return_value = make_mock_http_response(429, headers={"Retry-After": "0"})
...
def test_f5_orchestrator_no_retry_on_400_or_401():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        ...
        if mock_post.call_count > 1:
            ...
            pytest.fail(f"Orchestrator retries on 400/401 client errors (should not retry). Calls: {mock_post.mock_calls}")
...
def test_f6_config_loads_urls_from_yaml():
    ...
    config = load_config()
    if not any(hasattr(config, attr) for attr in ["grok_url", "claude_url", "codex_url", "urls", "services"]):
...
def test_f6_orchestrator_cleans_workspace():
    ...
    try:
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
             patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
            mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
            mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "new research"})
...
def test_f6_orchestrator_invalid_workspace_raises_error():
    ...
    with patch("os.makedirs", side_effect=mock_makedirs):
        with pytest.raises(PipelineError):
            asyncio.run(run_pipeline(prompt="test", workspace="/non_existent/directory/12345/abc"))
...
def test_f1_serve_py_interactive_prompt_selection():
    ...
    with patch("builtins.input", return_value="1"):
        assert hasattr(serve, "interactive_prompt") or hasattr(serve, "interactive_menu")
...
def test_f2_auth_key_extremely_long():
    ...
    client = TestClient(api.app)
    long_key = "a" * 8000
    res = client_post(client, "/run", headers={"X-API-Key": long_key}, json_data={"prompt": "hello"})
...
def test_f2_auth_expired_or_revoked_key():
    ...
    with patch.dict(os.environ, {"VALID_API_KEYS": ""}):
        res = client_post(client, "/run", headers={"X-API-Key": "expired-key"}, json_data={"prompt": "hello"})
        assert res.status_code == 401
...
def test_f2_auth_empty_key_header():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_post(client, "/run", headers={"X-API-Key": ""}, json_data={"prompt": "hello"})
    assert res.status_code == 401
...
def test_f2_auth_special_chars_in_key():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_post(client, "/run", headers={"X-API-Key": "key_#$@_123"}, json_data={"prompt": "hello"})
...
def test_f3_payload_extremely_large_prompt():
    ...
    res = client_post(client, "/run", headers={"X-API-Key": get_valid_api_key()}, json_data={"prompt": large_prompt})
...
def test_f3_payload_empty_prompt_and_context():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_post(client, "/run", headers={"X-API-Key": get_valid_api_key()}, json_data={"prompt": "", "context": ""})
...
def test_f3_status_invalid_task_id():
    ...
    from fastapi.testclient import TestClient
    client = TestClient(api.app)
    res = client_get(client, "/status/unknown-task-id-999", headers={"X-API-Key": get_valid_api_key()})
...
def test_f4_orchestrator_extremely_large_pipeline_response():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": large_content})
...
def test_f4_orchestrator_agent_returns_empty_result():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": ""})
        with pytest.raises(PipelineError):
...
def test_f4_orchestrator_invalid_port_in_url():
    ...
    check_orchestrator_rewritten()
    from orchestrator import run_pipeline
    with patch("httpx.AsyncClient.post", side_effect=httpx.InvalidURL("Invalid port range")):
        with pytest.raises(Exception):
            asyncio.run(run_pipeline(prompt="invalid port", grok_url="http://localhost:999999"))
...
def test_f4_orchestrator_agent_disconnects_mid_polling():
    ...
    from orchestrator import run_pipeline
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", side_effect=httpx.ConnectError("Connection lost")), \
         patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        with pytest.raises(PipelineError):
...
def test_f5_429_massive_retry_after():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        mock_post.return_value = make_mock_http_response(429, headers={"Retry-After": "999999"})
...
def test_f6_orchestrator_concurrent_pipeline_runs():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "output"})
...
def test_t3_api_key_rotation_during_pipeline():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "rotated"})
...
def test_t3_rate_limiting_triggered_sequentially():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "success"})
...
def test_t3_workspace_cleanup_on_http_failure():
    ...
    try:
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
             patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            mock_post.side_effect = httpx.HTTPStatusError("Error", request=None, response=make_mock_http_response(500))
            
            with pytest.raises(PipelineError):
                asyncio.run(run_pipeline(prompt="HTTP fail clean", workspace=workspace_dir))
...
def test_t3_swagger_docs_reflect_correct_security_schemas():
    ...
    if not security_schemes or not any(x in str(security_schemes) for x in ["ApiKeyAuth", "X-API-Key"]):
...
def test_t4_real_world_successful_microservices_pipeline():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
         patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        mock_post.return_value = make_mock_http_response(200, {"task_id": "real-task-1", "status": "processing"})
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "Genuine final output"})
        
        workspace_dir = "temp_workspace_t4_success"
        os.makedirs(workspace_dir, exist_ok=True)
        try:
            asyncio.run(run_pipeline(prompt="Deploy production microservices", workspace=workspace_dir))
            for f in ["research.md", "design.md", "app.py", "review.md", "test_generated.py"]:
                assert os.path.exists(os.path.join(workspace_dir, f))
        finally:
            shutil.rmtree(workspace_dir, ignore_errors=True)
...
def test_t4_real_world_network_jitter_recovery():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_post.side_effect = [
            ...
            make_mock_http_response(200, {"task_id": "jitter-task", "status": "processing"}),
        ] + [make_mock_http_response(200, {"task_id": "jitter-task", "status": "processing"})] * 5
        
        mock_get.side_effect = [
            httpx.ConnectError("Jitter"),
            make_mock_http_response(200, {"status": "processing"}),
            make_mock_http_response(200, {"status": "completed", "result": "recovered output"}),
        ] * 6
...
def test_t4_real_world_empty_design_payload_aborts():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_post.return_value = make_mock_http_response(200, {"task_id": "t-1", "status": "processing"})
        mock_get.side_effect = [
            make_mock_http_response(200, {"status": "completed", "result": "research content"}),
...
def test_t4_real_world_unauthorized_agent_aborts_pipeline():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post, \
        ...
        mock_get.return_value = make_mock_http_response(200, {"status": "completed", "result": "step 1 results"})
        with pytest.raises(PipelineError):
